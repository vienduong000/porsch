...
<?php 
	...
	*/
	class brand
	{
		...
		public function insert_comment($phone, $email){
			//$brandName = $this->fm->validation($phone, $number); //gọi ham validation để ktra có rỗng hay ko để ktra
			//$brandName = mysqli_real_escape_string($this->db->link, $phone, $number);
			 //mysqli gọi 2 biến. (catName and link) biến link -> gọi conect db từ file db
			...
			if(empty($phone)){
				...
				$query = "INSERT INTO tblpost(phone, email) VALUES('$phone', '$email') ";
				$result = $this->db->insert($query);
				if($result){
					$alert = "<span class='success'>Send infomation Successfully</span>";
					return $alert;
				}else {
					$alert = "<span class='error'>Send infomation NOT Success</span>";
					return $alert;
				}
			}
		}
